...
int main()
{
    ...
    availableCommands(); // Optional: show command options
...
void alarmActivationUpdate()
{
    ...
    static bool alarmLastState = false;  
    ...
    if (!overTempDetectorState && !gasDetectorState && !alarmTestButton) {
        alarmState = OFF;
    }

    
    if (alarmState && !alarmLastState) {  // Rising edge: OFF → ON
        arrayOfStoredEvents[eventsIndex].seconds = time(NULL);
        strcpy(arrayOfStoredEvents[eventsIndex].typeOfEvent, "ALARM_TRIGGERED");
        char str[100];
        sprintf(str, "Alarm Triggered at: %s", ctime(&arrayOfStoredEvents[eventsIndex].seconds));
            ...
            if (incorrectCodeLed) {
                ...
                if (alarmState) {
                    ...
                    for (int i = 0; i < EVENT_MAX_STORAGE; i++) {
                        if (arrayOfStoredEvents[i].seconds != 0) { // Check if event is valid
